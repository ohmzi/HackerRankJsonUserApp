...
data class ChatCompletionRequest(val model: String, val messages: List<ChatMessage>, val temperature: Double = 0.0)
data class ChatChoice(val index: Int, val message: ChatMessage, val finish_reason: String)
data class ChatCompletionResponse(val id: String, val `object`: String, val created: Long, val choices: List<ChatChoice>)
...
suspend fun getGenderForName(name: String, apiKey: String): String? {
    ...
    val prompt = "Determine whether the following name is typically male or female: \"$name\". Respond with either 'male' or 'female' only."
    ...
    return try {
        ...
        if (response.isSuccessful) {
            val answer = response.body()?.choices?.firstOrNull()?.message?.content?.trim()?.toLowerCase()
            when {
